...
UM.plugins['keystrokes'] = function() {
    ...
    me.addListener('keydown', function(type, evt) {
        ...
        if (keyCode == 8) {      	
        	//记录填空的数量 by lkl
            me.setOpt("blankNum",me.$container.find("input.blank").length,true);
    ...
    me.addListener('keyup', function(type, evt) {
        ...
        if(keyCode == 8){
            ...
            //删除填空时的处理 by lkl
            if(me.getOpt("blankNum") > me.$container.find("input.blank").length)
            {
                me.setOpt("blankNum",me.$container.find("input.blank").length,true);
                if(me.getOpt("afterBlankDel")!=null && typeof me.getOpt("afterBlankDel")=="function")
                {
                    var blankIds = [];
                    me.$container.find("input.blank").each(function(i,n){
                        blankIds.push(n.id);
                    });
                    me.getOpt("afterBlankDel")(blankIds);
                }
            }
        }
